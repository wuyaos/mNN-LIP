...
class ModelSummary:
    ...
    def __init__(self, model, max_depth: int = 1) -> None:
        ...
        # 1 byte -> 8 bits
        # mixed_precision: 'fp16', False, 'bf16'
        precision = 16 if isinstance(self._model.mixed_precision, str) else 32
...
def _is_lazy_weight_tensor(p: Tensor) -> bool:
    ...
    if isinstance(p, UninitializedParameter):
        # warning
        print("A layer with UninitializedParameter was found. "
            "Thus, the total number of parameters detected may be inaccurate.")
        return True
    return False


def summarize(module, max_depth: int = 1) -> ModelSummary:
