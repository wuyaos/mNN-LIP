...
def neighbor_pairs(pbc, coordinates, species, cell, cutoff: float, neigh_atoms: int):
    ...
    cell = cell.detach()
    num_repeats = [pbc[i] * math.ceil(cutoff / torch.max(torch.abs(cell[:, i]))).to(cell.device) for i in range(3)]
    ...
    distances = (selected_coordinates[:, 0, ...] - selected_coordinates[:, 1, ...] +
                 shifts_all).norm(2, -1)
    padding_mask = padding_mask.index_select(1, p12_all.view(-1)).view(num_mols, 2, -1).any(1)
    distances.masked_fill_(padding_mask, math.inf)  # dim=num_mols*(nshift*natom*natom)
    atom_index = torch.zeros((2, num_mols, num_atoms * neigh_atoms),
                             device=cell.device,
                             dtype=torch.long)
    shifts = -1e11 * torch.ones((num_mols, num_atoms * neigh_atoms, 3), device=cell.device)
    maxneigh = 0
    for inum_mols in range(num_mols):
        pair_index = torch.nonzero(
            ((distances[inum_mols] <= cutoff) * (distances[inum_mols] > 0.01))).reshape(-1)
    ...
    atom_index = atom_index.view(2, -1)
    shifts = shifts.view(-1, 3)
