...
PairNNLIP::PairNNLIP(LAMMPS *lmp) : Pair(lmp)
{
}
...
void PairNNLIP::init_style()
{
    neighbor->add_request(this, NeighConst::REQ_FULL);
}
// pair_coeff * * cutoff float TaHe.pt He Ta
// pair_coeff * * cutoff float TaHe.pt * Ta
// pair_coeff * * cutoff float TaHe.pt He *
void PairNNLIP::coeff(int narg, char **arg)
{
    if (narg < 7)
    ...
    // 判断系统类型：He Ta, He *->0, * Ta->1 args[5] args[6]
    if (strcmp(arg[5], "*") == 0)
        system_type = 1;
    ...
    //  check potential_file
    std::string pot_file = utils::get_potential_file_path(arg[4]);
    if (pot_file.empty())
        error->all(FLERR, "Cannot open potential parameter file {}", pot_file);
    try
    {
        ...
        if (datatype == "double")
            module = torch::jit::load(pot_file);
        else
        {
            module = torch::jit::load(pot_file);
...
void PairNNLIP::settings(int narg, char **arg)
{
}

double PairNNLIP::init_one(int i, int j)
{
    return cutoff;
}
...
void PairNNLIP::compute(int eflag, int vflag)
{
    if (eflag || vflag)
    ...
    for (ii = 0; ii < inum; ++ii)
    {
        ...
        ytmp = x[i][1];
        ztmp = x[i][2];
        local_species[i] = type[i] - 1 + system_type;
        jnum = numneigh[i];
        jlist = firstneigh[i];
        for (jj = 0; jj < jnum; ++jj)
        {
            ...
            if (d2 < cutoffsq)
            {
                ...
                neigh_species[totneigh] = type[j] - 1 + system_type;
                ++totneigh;
            }
        }
    }
...
int PairNNLIP::select_gpu()
{
    ...
    int systemRet = system("nvidia-smi -q -d Memory |grep -A4 GPU|grep Used >gpu_info");
    if (systemRet == -1)
    {
